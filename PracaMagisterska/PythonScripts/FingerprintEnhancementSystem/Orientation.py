...
class Orientation():
    ...
    def decomposeImage(self,theta,ksize):
        ...
        for wins in range (0,4):
            ...
            if not orient[wins].shape[0] == 0:
                ...
                angleAvr = (np.arctan2(point1[1]- point2[1], point1[0] - point2[0]))
                ker = cv2.getGaborKernel((ksize,ksize),4,angleAvr,12,2,cv2.CV_32F)
                print "windows[wins].shape"
                print windows[wins].shape
                # print windows[wins]
                # windows[wins] = cv2.filter2D(windows[wins],cv2.CV_32F,ker)
                # windows[wins] = self.power(windows[wins], ker)
                if np.any(x0):
